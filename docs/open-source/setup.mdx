...
description: "Stand up the Ram0 REST server and dashboard in a few minutes, with an admin account, API keys, and a live audit log included."
icon: "rocket-launch"
---
...
<Info>
  **Use this page when…**
  - You want self-hosted Ram0 with a dashboard, not just the Python or Node library.
...
| **Entities** | The authenticated owner's distinct `agent_id` / `run_id` values with memory counts and owner-scoped deletion; public `user_id` entities are not exposed. |
