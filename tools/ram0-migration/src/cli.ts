...
import { SupermemoryCoverageBackend, verifyImportedPlan } from "./verify"
...
function mapping(value: unknown): ContainerMapping {
	...
	if (
		typeof personalContainer !== "string" ||
		!appIds ||
		typeof appIds !== "object" ||
...
export async function run(
	...
	if (command === "import") {
		...
		if (currentPlan.invalid.length > 0)
			throw new Error("import plan contains invalid records")
	...
	if (command === "stats") {
		...
		return {
			...
			containers: [
				...new Set(currentPlan.records.map((record) => record.containerTag)),
			].sort(),
		}
	}
	if (command === "verify") {
		...
		const result = await verifyImportedPlan(
			currentPlan,
			new SupermemoryCoverageBackend(option(args, "--base-url"), apiKey),
		)
		if (!result.complete)
			throw new Error(
				`import verification is incomplete: ${JSON.stringify(result.containers)}`,
			)
		return result
	}
